...
if ( ! function_exists( 'jws_add_to_wishlist_btn' ) ) {
	...
	function jws_add_to_wishlist_btn() {
		...
		echo '<a href="' . esc_url( jws_get_wishlist_page_url() ) . '" class="jws-wishlist-btn fs-small fw-700 cl-heading'.esc_attr($class_added).'" data-id="' . esc_attr( $product->get_id() ) . '">
            <span class="notadd">'.esc_html__('Add to wishlist','streamvid').'</span>
            <span class="added">'.esc_html__('Browse Wishlist','streamvid').'</span>
        </a>';
	}
}
...
if ( ! function_exists( 'jws_add_to_wishlist_single_btn' ) ) {
	...
	function jws_add_to_wishlist_single_btn() {
		global $product;
		echo '<a class="jws-wishlistsg-btn" data-added-text="' . esc_html__('Wishlist products', 'streamvid') . '" data-id="' . esc_attr( $product->get_id() ) . '"><span class="jws-icon-heart-straight-thin"></span></a>';
	}
}
			...
			if ( ! empty( $products ) ) {
						...
						<table class="jws-wishlist-row">
                            <thead>
                    			<tr>
                    				...
                    				<th class="title_wishlist"><?php echo esc_html__('Product Detail','streamvid'); ?></th>
                                    <th class="title_wishlist"><?php echo esc_html__('Price','streamvid'); ?></th>
                    				<th class="title_wishlist"><?php echo esc_html__('Stock Status','streamvid'); ?></th>
                                    <th class="title_wishlist"></th>
                    			</tr>
                			</thead>
                            <tbody>
				...
				?>
					<h4 class="jws-empty-wishlist">
						<?php esc_html_e('Wishlist is empty.', 'streamvid'); ?>
					</h4>
					...
					<p class="return-to-shop">
						<a class="button button-default" href="<?php echo esc_url( wc_get_page_permalink( 'shop' ) ); ?>">
							<?php esc_html_e( 'Return to shop', 'streamvid' ); ?>
						</a>
					</p>
				<?php
			}
...
if ( ! function_exists( 'jws_wishlist_display_field' ) ) {
	...
	function jws_wishlist_display_field( $field_id, $product ) {
		...
		switch ( $type ) {
			case 'basic':
                    echo '<td><a href="#" class="jws-wishlist-remove" data-id="' . esc_attr( $product['id'] ) . '"><span class="jws-icon-x"></span></a></td>';
...
if ( ! function_exists( 'jws_wishlist_availability_html' ) ) {
	...
	function jws_wishlist_availability_html( $product ) {
		...
		if( empty( $availability['availability'] ) ) {
			$availability['availability'] = esc_html__( 'In stock', 'streamvid' );
		}
