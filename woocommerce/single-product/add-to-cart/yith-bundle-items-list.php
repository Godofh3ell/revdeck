				...
				<div class="yith-wcpb-product-bundled-item-image">
					...
					if ( ! $hide_thumbnail ) {
						...
						if ( $post_thumbnail_id ) {
							...
							echo sprintf( '<img src="%s" alt="%s" class="wp-post-image" />', esc_url( wc_placeholder_img_src() ), esc_html__( 'Awaiting product image', 'streamvid' ) );
							echo '</div>';
						}
					}
							...
							<div class="yith-wcpb-bundled-quantity__invalid-notice">
								...
								echo wp_kses_post(
									...
									// translators: 1. the minimum amount; 2. the maximum amount.
										__( 'You can choose a minimum quantity of %1$s and a maximum quantity of %2$s for this product!', 'streamvid' ),
								...
								<?php foreach ( $b_attributes as $name => $options ) : ?>
									<tr>
										...
										<td class="value">
											...
											echo end( $attribute_keys ) === $name ? wp_kses_post( apply_filters( 'woocommerce_reset_variations_link', '<a class="reset_variations" href="#">' . esc_html__( 'Clear', 'streamvid' ) . '</a>' ) ) : '';
											?>
										</td>
									</tr>
								<?php endforeach; ?>
