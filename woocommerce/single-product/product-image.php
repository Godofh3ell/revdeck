...
defined( 'ABSPATH' ) || exit;
        ...
        <div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $wrapper_classes ) ) ); ?>"  style="opacity: 0; transition: opacity .25s ease-in-out;">
			...
			<?php
				...
				if ( $product->get_image_id() ) {
					...
					$html .= sprintf( '<img src="%s" alt="%s" data-src="%s" data-large_image="%s" data-large_image_width="700" data-large_image_height="800" class="attachment-woocommerce_single size-woocommerce_single wp-post-image" />', esc_url( wc_placeholder_img_src( $placeholder_size ) ), esc_html__( 'Awaiting product image', 'streamvid' ), esc_url( wc_placeholder_img_src( $placeholder_size ) ), esc_url( wc_placeholder_img_src( $placeholder_size ) ) );
        ...
        <?php 
             if (!empty($video) && $video_type == 'popup') : 
                ...
                echo '<a href="' . esc_url($video) . '" class="action-popup-url"><i aria-hidden="true" class="jws-icon-play-circle"></i>' . esc_html__('View video', 'streamvid') . '</a>';
