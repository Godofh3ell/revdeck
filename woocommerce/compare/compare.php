...
if ( ! function_exists( 'jws_add_to_compare_btn' ) ) {
	...
	function jws_add_to_compare_btn() {
		...
		echo '<a href="' . esc_url( jws_get_compare_page_url() ) . '" class="button jws-compare-btn'.esc_attr($class_added).'" data-id="' . esc_attr( $product->get_id() ) . '">
            ...
            <span class="notadd">'.esc_html__('Add to compare','streamvid').'</span>
            <span class="added">'.esc_html__('View compare','streamvid').'</span>
        </a>';
	}
}
...
if ( ! function_exists( 'jws_add_to_compare_single_btn' ) ) {
	...
	function jws_add_to_compare_single_btn() {
		global $product;
		echo '<a class="jws-comparesg-btn" data-added-text="' . esc_html__('Compare products', 'streamvid') . '" data-id="' . esc_attr( $product->get_id() ) . '"><span class="jws-icon-arrow_left-right_alt"></span></a>';
	}
}
			...
			if ( ! empty( $products ) ) {
				...
				?>
					<p class="jws-empty-compare">
						<?php esc_html_e('Compare list is empty.', 'streamvid'); ?>
					</p>
...
if ( ! function_exists( 'jws_compare_display_field' ) ) {
	...
	function jws_compare_display_field( $field_id, $product ) {
		...
		switch ( $type ) {
		    case 'remove':
					echo '<a href="#" class="jws-compare-remove" data-id="' . esc_attr( $product['id'] ) . '"><span class="remove-loader"></span>' . esc_html__( 'Remove', 'streamvid' ) . '</a>';
			break;  
			case 'basic':
...
if ( ! function_exists( 'jws_compare_get_field_name_by_id' ) ) {
	...
	function jws_compare_get_field_name_by_id( $id ) {
		$fields = array(
            'remove' => esc_html__( 'Ext', 'streamvid' ),
            'basic' => esc_html__( 'Products', 'streamvid' ),
			'description' => esc_html__( 'Description', 'streamvid' ),
			'sku' => esc_html__( 'Sku', 'streamvid' ),
			'availability' => esc_html__( 'Availability', 'streamvid' ),
			'weight' => esc_html__( 'Weight', 'streamvid' ),
			'dimensions' => esc_html__( 'Dimensions', 'streamvid' ),
		);
